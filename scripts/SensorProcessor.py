...
class CamProcessing:
    ...
    def matchDescript(self,current_frame,last_frame):
    ...
    def matchBBox(self, good_kp_current,good_kp_last,bBoxes_current,bBoxes_last):    
        '''
        ...
        for idx, (kp_current,kp_last) in enumerate(zip(good_kp_current,good_kp_last)): 
            ...
            for idx_box, box in enumerate(bBoxes_current):
                if self.boxContain(box.roi,kp_current):
                    current_box_idx=idx_box                      
            for idx_box, box in enumerate(bBoxes_last):
                if self.boxContain(box.roi,kp_last):
        ...
        for idx_box, box in enumerate(bBoxes_current):
            for idx_keypoint, keypoint in enumerate(good_kp_current):
                if self.boxContain(box.roi,keypoint):
    ...
    def clusterWithROI(self,box_current,good_kp_current,good_kp_last,good_matches):
        ...
        for kp_current,kp_last,match in zip(good_kp_current,good_kp_last,good_matches):
            ...
            if np.abs(dist_match-dist_mean)<dist_std:
                if self.boxContain(box_current.roi,kp_current):
    ...
    def computeCam(self,currBox,frameRate):
        '''
...
class PointProcessing:
    ...
    def projectPointToImage(self,lidar_pts): 
    ...
    def clusterPointROI(self, bBoxes,lidar_px,lidar_pts):  
        #lidar_px: (2,n)
        #lidar_pts=(n,4)
        #bBoxes is a list,it will be modified here
        select_bool=np.ones(lidar_px.shape[1],dtype=bool) #1: can be selected        
        for box in bBoxes:
            inside_box,select_bool=self.boxContain(box.roi,np.copy(lidar_px), select_bool)
    ...
    def boxContain(self,roi, lidar_px,select_bool):
        '''roi=:[x,y,w,h]
    ...
    def veloFilter(self,points, v_fov, h_fov, max_d):
        ...
        x_lim = self._fovSetting(x, x, y, z, dist, h_fov, v_fov)[:,None]
        y_lim = self._fovSetting(y, x, y, z, dist, h_fov, v_fov)[:,None]
        z_lim = self._fovSetting(z, x, y, z, dist, h_fov, v_fov)[:,None]   
        ...
        dist_lim = self._fovSetting(dist, x, y, z, dist, h_fov, v_fov)
        color = self._depthColor(dist_lim, 0, max_d)    
        return lidar_pts, color    
    
    def cropPoint(self,lidar_pts,crop_range,max_d):
    ...
    def computeLidar(self,currBox,lastBox, frameRate):
    ...
    def _depthColor(self,val, min_d=0, max_d=120):
        """ 
    ...
    def _inHrangePoint(self,points, m, n, fov):
        """ extract horizontal in-range points """
        return np.logical_and(np.arctan2(n,m) > (-fov[1] * np.pi / 180), \
                              np.arctan2(n,m) < (-fov[0] * np.pi / 180))
    
    def _inVrangePoint(self,points, m, n, fov):
        """ extract vertical in-range points """
        return np.logical_and(np.arctan2(n,m) < (fov[1] * np.pi / 180), \
                              np.arctan2(n,m) > (fov[0] * np.pi / 180))
    
    def _fovSetting(self,points, x, y, z, dist, h_fov, v_fov):
        ...
        if h_fov[1] == 180 and h_fov[0] == -180:
            return points[self._inVrangePoint(points, dist, z, v_fov)]
        elif v_fov[1] == 2.0 and v_fov[0] == -24.9:        
            return points[self._inHrangePoint(points, x, y, h_fov)]
        else:
            h_points = self._inHrangePoint(points, x, y, h_fov)
            v_points = self._inVrangePoint(points, dist, z, v_fov)
