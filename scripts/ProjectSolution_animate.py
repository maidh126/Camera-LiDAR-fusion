...
def animate(idx):
    ...
    if len(dataBuffer)>1:    
        ...
        good_matches,good_kp_current,good_kp_last=CP.matchDescript(current_frame,last_frame)
        ...
        current_frame.bbMatches=CP.matchBBox(good_kp_current,good_kp_last,current_frame.boundingBoxes,last_frame.boundingBoxes)  
     
        #consider boxID index
        for box in current_frame.boundingBoxes:
            color = (list(np.random.choice(range(256), size=3)))  
            color_kp =[int(color[0]), int(color[1]), int(color[2])]  
            CP.clusterWithROI(box,good_kp_current,good_kp_last,good_matches)
        ...
        for match in current_frame.bbMatches:
            ...
            TTC_cam=CP.computeCam(currBox,params["framerate"])
            TTC_lidar=LP.computeLidar(currBox,lastBox,params["framerate"])
